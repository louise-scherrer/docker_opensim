...
1. To use it:
   1. Run `host +local:` to give permission to use the X11 socket for GUI display
   1. Use `./start-opensim.sh run opensim` to run opensim from the published image ghcr.io/louise-scherrer/opensim:main
   1. Or use `./start-opensim.sh run opensim-dev` to build a local container and run opensim
   1. Or use `./start-opensim.sh run opensim bash` to enter a shell script
